...
const PRODUCT_FORM_ID = "electronics-new-entry-form";

export default function ElectronicsNewEntryPage() {
  const { categories, brands, models, addProduct } = useElectronicsData();
  const { showToast } = useToast();
  const [formKey, setFormKey] = useState(0);

  function handleSubmit(values: ProductFormOutput) {
    addProduct({ id: generateId("ep"), ...values });
    showToast(`"${values.name}" was added to your Electronics catalog.`, "success");
    setFormKey((k) => k + 1);
  }
  ...
  return (
    <div>
      <Breadcrumbs
        items={[
          { label: "Electronics", href: "/inventory/electronics/dashboard" },
          { label: "New Entry" },
        ]}
      />
      <PageHeader
        title="New Product Entry"
        description="Add a new product to your electronics inventory."
      />

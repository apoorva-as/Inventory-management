...
const PAGE_SIZE = 8;
const CUSTOMER_FORM_ID = "electronics-customer-form";

export default function ElectronicsCustomersPage() {
  const { customers, sales, returns, addCustomer, updateCustomer, deleteCustomer } = useElectronicsData();
  const { showToast } = useToast();

  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  const [panel, setPanel] = useState<{ mode: "add" | "edit"; customer?: ElectronicsCustomer } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<ElectronicsCustomer | null>(null);
  ...
  function handleFormSubmit(values: CustomerFormOutput) {
    if (panel?.mode === "edit" && panel.customer) {
      ...
      addCustomer({ id: generateId("ecu"), totalOrders: 0, totalSpent: 0, ...values });
  ...
  function confirmDelete() {
    ...
    const hasRecords =
      ...
      returns.some((r) => r.customerId === deleteTarget.id);
    if (hasRecords) {
      showToast(`Can't delete "${deleteTarget.name}" — they have sale or return records on file.`, "error");
  ...
  const columns: DataTableColumn<ElectronicsCustomer>[] = [
    {
      key: "name",
      header: "Customer",
      render: (c) => (

...
import { VendorForm, type VendorFormOutput } from "@/components/electronics/VendorForm";
import { useElectronicsData } from "@/lib/context/ElectronicsDataProvider";
import { generateId } from "@/lib/utils/id";
import type { ElectronicsVendor } from "@/lib/types/electronics";

const PAGE_SIZE = 8;
const VENDOR_FORM_ID = "electronics-vendor-form";

export default function ElectronicsVendorsPage() {
  const { vendors, purchases, addVendor, updateVendor, deleteVendor } = useElectronicsData();
  const { showToast } = useToast();

  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  const [panel, setPanel] = useState<{ mode: "add" | "edit"; vendor?: ElectronicsVendor } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<ElectronicsVendor | null>(null);
  ...
  function handleFormSubmit(values: VendorFormOutput) {
    if (panel?.mode === "edit" && panel.vendor) {
      ...
      addVendor({ id: generateId("ev"), ...values });
  ...
  const columns: DataTableColumn<ElectronicsVendor>[] = [
    {
      key: "name",
      header: "Vendor",
      render: (v) => (

...
import { SaleForm, type SaleFormOutput } from "@/components/electronics/SaleForm";
...
const PAGE_SIZE = 8;
const SALE_FORM_ID = "electronics-sale-form";
...
export default function ElectronicsSalesPage() {
  const { sales, customers, products, addSale } = useElectronicsData();
  ...
  const [detailsTarget, setDetailsTarget] = useState<ElectronicsSale | null>(null);
  ...
  function handleFormSubmit(values: SaleFormOutput) {
    addSale({ id: generateId("esa"), ...values });
    showToast(`Sale to "${values.customerName}" was recorded.`, "success");
    setFormOpen(false);
  }

  const columns: DataTableColumn<ElectronicsSale>[] = [
    {
      key: "id",
      header: "Sale",
      render: (s) => (

...
const PAGE_SIZE = 8;
const CUSTOMER_FORM_ID = "grocery-customer-form";

export default function GroceryCustomersPage() {
  const { customers, sales, addCustomer, updateCustomer, deleteCustomer } = useGroceryData();
  const { showToast } = useToast();

  const [search, setSearch] = useState("");
  const [page, setPage] = useState(1);
  const [panel, setPanel] = useState<{ mode: "add" | "edit"; customer?: GroceryCustomer } | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<GroceryCustomer | null>(null);
  ...
  function handleFormSubmit(values: CustomerFormOutput) {
    if (panel?.mode === "edit" && panel.customer) {
      ...
      addCustomer({ id: generateId("gc"), totalOrders: 0, totalSpent: 0, ...values });
  ...
  function confirmDelete() {
    ...
    if (hasSales) {
      showToast(`Can't delete "${deleteTarget.name}" — they have sale records on file.`, "error");
  ...
  const columns: DataTableColumn<GroceryCustomer>[] = [
    {
      key: "name",
      header: "Customer",
      render: (c) => (

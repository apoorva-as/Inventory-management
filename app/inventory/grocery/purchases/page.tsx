...
import { PurchaseForm, type PurchaseFormOutput } from "@/components/grocery/PurchaseForm";
...
const PAGE_SIZE = 8;
const PURCHASE_FORM_ID = "grocery-purchase-form";
...
export default function GroceryPurchasesPage() {
  const { purchases, vendors, products, addPurchase } = useGroceryData();
  ...
  const [detailsTarget, setDetailsTarget] = useState<GroceryPurchase | null>(null);
  ...
  function handleFormSubmit(values: PurchaseFormOutput) {
    addPurchase({ id: generateId("gpu"), ...values });
    showToast(`Purchase from "${values.vendorName}" was recorded.`, "success");
    setFormOpen(false);
  }

  const columns: DataTableColumn<GroceryPurchase>[] = [
    {
      key: "id",
      header: "Purchase",
      render: (p) => (

...
import type { GroceryBrand } from "@/lib/types/grocery";

interface BrandRow extends GroceryBrand {
  productCount: number;
}

export default function GroceryBrandsPage() {
  const { brands, products, addBrand, updateBrand, deleteBrand } = useGroceryData();
  const { showToast } = useToast();

  const [search, setSearch] = useState("");
  const [editTarget, setEditTarget] = useState<GroceryBrand | "new" | null>(null);
  ...
  function openEdit(brand: GroceryBrand) {
  ...
  function handleSave() {
    ...
    if (editTarget === "new") {
      addBrand({ id: generateId("brand"), name });
  ...
  return (
    <div>
      <PageHeader
        title="Brands"
        description={`${brands.length} brands in your grocery catalog`}
        actions={

...
import type { MedicalCategory } from "@/lib/types/medical";

interface CategoryRow extends MedicalCategory {
  medicineCount: number;
}

export default function MedicalCategoriesPage() {
  const { categories, medicines, addCategory, updateCategory, deleteCategory } = useMedicalData();
  const { showToast } = useToast();

  const [search, setSearch] = useState("");
  const [editTarget, setEditTarget] = useState<MedicalCategory | "new" | null>(null);
  ...
  const rows: CategoryRow[] = useMemo(() => {
    ...
    return categories
      .map((c) => ({ ...c, medicineCount: medicines.filter((m) => m.categoryId === c.id).length }))
      .filter((c) => !term || c.name.toLowerCase().includes(term));
  }, [categories, medicines, search]);
  ...
  function openEdit(category: MedicalCategory) {
  ...
  function handleSave() {
    ...
    if (editTarget === "new") {
      addCategory({ id: generateId("cat"), name });
  ...
  const columns: DataTableColumn<CategoryRow>[] = [
    ...
    { key: "medicineCount", header: "Medicines", render: (c) => `${c.medicineCount}` },
    {
      key: "actions",
      header: "",
  ...
  return (
    <div>
      <PageHeader
        title="Categories"
        description={`${categories.length} medicine categories`}
        actions={
      ...
      >
        ...
        <input
          ...
          placeholder="e.g. Pain Relief & Analgesics"
        />
      </Modal>
      ...
      >
        <p className="text-sm text-muted">
          ...
          {deleteTarget && deleteTarget.medicineCount > 0 && (
            <> This category is used by {deleteTarget.medicineCount} medicine(s).</>
          )}
        </p>
      </Modal>
    </div>

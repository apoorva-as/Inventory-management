...
import type { MedicalMedicine, MedicalSupplier, PurchaseOrderLine, PurchaseOrderStatus } from "@/lib/types/medical";
import { cn } from "@/lib/utils/cn";
import { formatCurrency } from "@/lib/utils/formatters";
import { Button } from "@/components/shared/Button";

export interface PurchaseOrderFormOutput {
  supplierId: string;
  supplierName: string;
  date: string;
  expectedDate?: string;
...
function newRow(): ItemRow {
  return { key: `row-${++rowIdCounter}`, medicineId: "", quantity: "1", purchasePrice: "" };
}
...
interface PurchaseOrderFormProps {
  ...
  onSubmit: (po: PurchaseOrderFormOutput) => void;
}
...
export function PurchaseOrderForm({ formId, suppliers, medicines, onSubmit }: PurchaseOrderFormProps) {
  ...
  function handleMedicineChange(key: string, medicineId: string) {
    ...
    updateRow(key, { medicineId, purchasePrice: medicine ? String(medicine.purchasePrice) : "" });
  }
  ...
  function validate(): boolean {
    ...
    const validRows = rows.filter((r) => r.medicineId && Number(r.quantity) > 0 && r.purchasePrice !== "");
    if (validRows.length === 0)
      nextErrors.items = "Add at least one medicine with a quantity and purchase price.";
  ...
  function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    const items: PurchaseOrderLine[] = rows
      .filter((r) => r.medicineId && Number(r.quantity) > 0 && r.purchasePrice !== "")
      .map((r) => {
    ...
    const computedTotal = items.reduce((sum, i) => sum + i.quantity * i.purchasePrice, 0);
  ...
  return (
    <form id={formId} onSubmit={handleSubmit} className="space-y-4" noValidate>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className={labelClass} htmlFor="po-expected-date">
            Expected Date
          </label>
          <input
            id="po-expected-date"
        ...
        <div>
          <label className={labelClass} htmlFor="po-status">
            Status
          </label>
          <select
            id="po-status"
          ...
          <p className="mt-1 text-xs text-muted">
            A purchase order never affects stock — only receiving it does.
          </p>
        </div>
      </div>
      ...
      <div>
        ...
        <div className="space-y-2">
          {rows.map((row) => {
            const lineTotal = (Number(row.quantity) || 0) * (Number(row.purchasePrice) || 0);
            return (
